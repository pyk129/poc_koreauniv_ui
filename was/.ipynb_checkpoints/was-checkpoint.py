...
class Recommend1:
    ...
    path_실거래가 = 'datas/dataApt실거래가.csv'
    
    path_2ndModel = 'datas/recommend2nd.pkl'
...
# =============================================================================
        ...
        self.df실거래가 = pd.read_csv(self.path_실거래가)
    ...
    def setup(self, simirarity, gu, optSecond, sel1, sel2, sel3, sel4, sel5, sel6, sel7,sel8, sel9):
         # [선택조건Index, 제한거리(km)]
...
# =============================================================================
            ...
            df2nd = self.df실거래가[self.df실거래가['도로명'].str.contains(juso)]
            
            predict = 0
            
            maemae_jisu = []
            if len(juso.replace(' ','')) > 0:                
            
                if len(df2nd) > 0:     
# =============================================================================
#                     print('juso  : ', juso, ' 실거래가도로명 : ', df2nd)
# =============================================================================
                    df2nd = df2nd[['전용면적(㎡)', '계약년월', '계약일', '거래금액(만원)', '매매가격지수','통화금융지표', '기준금리']]
                    
                    dfmax = df2nd.max()
    
# =============================================================================
#                     redict = self.clf_from_joblib.predict([df2nd])
# =============================================================================
                    maemae_jisu_lastItem = {}
                    for i, d in df2nd.iterrows():                  
# =============================================================================
#                          print(d)
# =============================================================================
                         jisuItem = {}
                         ...
                         predict = self.clf_from_joblib.predict([d])
#                         print('** : ', predict['매매가격지수'])
                         jisuItem['predict_jisu'] = str(predict[0])
...
# =============================================================================
                    ...
                    # 1개월 뒤만 표기 
                    maemae_jisu.append(createPredict2NdDataAfterMonth(last2NdItem.iloc[0], 1))
...
# =============================================================================
                    ...
                    predict = self.clf_from_joblib.predict([dfmax])

            predict2nd.append(predict)
...
#
    ...
    recommend.setup("1", d['gu'], d['optSecond'], d['select1'], d['select2'],d['select3'],d['select4'],d['select5'],d['select6'],d['select7'],d['select8'],d['select9'])
